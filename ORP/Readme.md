...
本UI自动化测试框架旨在帮助团队快速编写和运行Web界面的自动化测试用例。框架采用了经典的Page
Object Model设计，以页面对象配合关键字驱动的方式封装常用操作。主要技术栈包括：
- **Selenium**: 作为底层浏览器操作库，驱动浏览器执行点击、输入等 UI 操作，支持 Chrome、Firefox 等常用浏览器。
- **Pytest**: 作为测试用例组织和执行框架，支持参数化测试、fixtures 注入等特性，简化测试初始化和清理。
- **pytest-dependency**: Pytest插件，建立脚本之间的依赖关系。
- **pytest-rerunfailures**: Pytest插件，脚本失败重试机制。增加测试稳定。
- **pytest-xdist**: Pytest插件，支持脚本并发执行测试用例。
- **pytest-image-snapshot**: Pytest插件，支持图像对比功能。
- **Allure**: 作为测试报告工具，生成美观的测试报告。框架通过 Allure 提供步骤注解和失败截图等增强报告的功能。
- **nb_log**: 日志库，用于记录详细运行日志到文件，便于调试和追踪。

框架适用于Web UI的功能回归测试和验证。通过封装常见操作（点击、输入、切换iframe、截图比对等），测
试人员无需直接调用Selenium原生API，只需使用框架提供的关键字方法即可实现稳定可靠的操作和断言。
---
## 项目结构总览
